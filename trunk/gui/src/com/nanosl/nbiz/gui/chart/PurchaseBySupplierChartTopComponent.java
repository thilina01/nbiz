 ...
 */
@ConvertAsProperties(
        dtd = "-//com.nanosl.nbiz.gui//PurchaseBySupplierChart//EN",
        autostore = false
)
@TopComponent.Description(
        preferredID = "PurchaseBySupplierChartTopComponent",
        //iconBase="SET/PATH/TO/ICON/HERE", 
        persistenceType = TopComponent.PERSISTENCE_ALWAYS
)
@TopComponent.Registration(mode = "editor", openAtStartup = false)
@ActionID(category = "Window", id = "com.nanosl.nbiz.gui.PurchaseBySupplierChartTopComponent")
@ActionReference(path = "Menu/Chart" /*, position = 333 */)
@TopComponent.OpenActionRegistration(
        displayName = "#CTL_PurchaseBySupplierChartAction",
        preferredID = "PurchaseBySupplierChartTopComponent"
)
@Messages({
    "CTL_PurchaseBySupplierChartAction=PurchaseBySupplierChart",
    "CTL_PurchaseBySupplierChartTopComponent=PurchaseBySupplierChart Window",
    "HINT_PurchaseBySupplierChartTopComponent=This is a PurchaseBySupplierChart window"
})
public final class PurchaseBySupplierChartTopComponent extends NTopComponent {

    public PurchaseBySupplierChartTopComponent() {
        onLoad();
        setName(Bundle.CTL_PurchaseBySupplierChartTopComponent());
        setToolTipText(Bundle.HINT_PurchaseBySupplierChartTopComponent());
    ...
    private void initComponents() {
        ...
        org.openide.awt.Mnemonics.setLocalizedText(jLabel1, org.openide.util.NbBundle.getMessage(PurchaseBySupplierChartTopComponent.class, "PurchaseBySupplierChartTopComponent.jLabel1.text")); // NOI18N

        topTextField.setText(org.openide.util.NbBundle.getMessage(PurchaseBySupplierChartTopComponent.class, "PurchaseBySupplierChartTopComponent.topTextField.text")); // NOI18N
    ...
    private void createChartScene() {
        Platform.runLater(new Runnable() {
            ...
            public void run() {
                ...
                String x = "SELECT supplier.`name`,ROUND(SUM(purchase_invoice.amount),2) as total "
                        + "FROM "
                        + "purchase_invoice "
                        + "INNER JOIN supplier ON purchase_invoice.supplier_code = supplier.`code` "
                        + "WHERE "
                        + "purchase_invoice.inv_date BETWEEN '" + yyyy_MM_dd.format(makeStartDate(datePicker.getDate())) + " 00:00:00' AND '" + yyyy_MM_dd.format(makeEndDate(datePicker1.getDate())) + " 23:59:59' "
...
//                table.getColumns().add(new TableColumn(resultSet.getString("date_only").substring(resultSet.getString("date_only").lastIndexOf("-") + 1)));
                        pieChartData.add(new PieChart.Data(resultSet.getString("name") + " " + nf2d.format(resultSet.getDouble("total")), resultSet.getDouble("total")));
                    }
                } catch (SQLException ex) {
                    Exceptions.printStackTrace(ex);
                }
                final PieChart chart = new PieChart(FXCollections.observableArrayList(pieChartData));
                chart.setTitle("Purchase of - " + yyyy_MM_dd.format(makeStartDate(datePicker.getDate())) + " to " + yyyy_MM_dd.format(makeEndDate(datePicker1.getDate())));
